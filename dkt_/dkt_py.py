...
class DKT(nn.Module):
    def __init__(self, n_question, embed_l, n_time_bins, n_time_bins_t, hidden_dim, num_layers=1, class_weights=None, final_fc_dim=512, dropout=0.0, z_weight=0.0, pretrained_embeddings=None, freeze_pretrained=True):
        super().__init__()
        """
        Input:
        ...
        self.n_time_bins_t = n_time_bins_t
        ...
        self.rnn = nn.LSTM(
            input_size=embed_l + 1 + num_features,  # word embedding, time bin, session num correct, session num attempted
        ...
        self.out = nn.Sequential(
            ...
            nn.Linear(hidden_dim + embed_l + num_features, 2048),
            nn.ReLU(),
            nn.Dropout(self.dropout),
            nn.Linear(2048, 1024),
            nn.ReLU(),
            nn.Dropout(self.dropout),
            nn.Linear(1024, final_fc_dim),
            nn.ReLU(),
            nn.Dropout(self.dropout),
    ...
    def forward(self, q_data, items_data, kcs_data, langs_data, lans_data, wordsize_data, correct_data, attempts_data, c_correct_data, c_attempts_data, delta, delta_t, time_bin_data, time_bin_t_data, target, mask):
        """
            input:
        ...
        kc_embed_data = self.kc_embed(kcs_data)
        # time_bin_categorical = to_categorical(time_bin_data, self.n_time_bins)
        # time_bin_t_categorical = to_categorical(time_bin_t_data, self.n_time_bins_t)
        # (200,256)
        ...
        #(batch size, seq_len, in_dim) = (256,200,300+6+1+1=308)
        ...
        rnn_input = torch.cat([kc_embed_data, torch.unsqueeze(langs_data, 2), torch.unsqueeze(wordsize_data, 2),torch.unsqueeze(target, 2), torch.unsqueeze(correct_data, 2),torch.unsqueeze(attempts_data, 2), torch.unsqueeze(delta, 2), torch.unsqueeze(delta_t, 2)], dim=2)


        hidden_seq, (hn,cn) = self.rnn(rnn_input, (h_0, c_0))
        #(256,200,2048)-->(1,200,2048)-->cat-->(1+256,200,2048)--->(256,200,2048)

        h = torch.cat([h_0[-1:, :, :], hidden_seq], dim=0)[:-1, :, :]  # T,BS,hidden_dim
        # h = hidden_seq
        #(256,200,2048+300+6=2354)
        ffn_input = torch.cat([h, kc_embed_data, torch.unsqueeze(langs_data, 2), torch.unsqueeze(wordsize_data, 2), torch.unsqueeze(correct_data, 2), torch.unsqueeze(attempts_data, 2), torch.unsqueeze(delta, 2), torch.unsqueeze(delta_t, 2)], dim=2)  # concatenate time-shifted hidden states with current question info
        #(256,200,1)
        ...
        loss = nn.BCEWithLogitsLoss(reduction='none') # sigmoid + BCEloss
...
class DKT_att(nn.Module):
    def __init__(self, n_question, embed_l, hidden_dim, layer_dim=1, class_weights=None, final_fc_dim=512, dropout=0.0, z_weight=0.0, pretrained_embeddings=None,  freeze_pretrained=True):
        ...
        self.n_user = 43805
        self.rnn = nn.LSTM(
            input_size= embed_l + 1 + num_features+emb_i,  # word embedding, time bin, session num correct, session num attempted
    ...
    def attention_net(self, lstm_output, state):
        # hidden = final_state.view(-1, self.hidden_dim , 1)   # hidden : [batch_size, n_hidden * num_directions(=2), 1(=n_layer)]
        state= state.transpose(1, 2)
        # print(len(lstm_output), len(lstm_output[0]),len(lstm_output[0][0]))
        # print(len(state), len(state[0]),len(state[0][0]))
        attn_weights = torch.bmm(lstm_output, state).squeeze(2) # attn_weights : [batch_size, n_step]
        soft_attn_weights = F.softmax(attn_weights, 1)
        # print(len(soft_attn_weights),len(soft_attn_weights[0]),len(soft_attn_weights[0][0]))
        # [batch_size, n_hidden * num_directions(=2), n_step] * [batch_size, n_step, 1] = [batch_size, n_hidden * num_directions(=2), 1]

        context = torch.bmm(lstm_output.transpose(1, 2), soft_attn_weights).squeeze(2)
        # print(len(lstm_output.transpose(1, 2)), len(soft_attn_weights))
        # print(len(context))
    ...
    def attention_net_q(self, q_context, state, l):
        ...
        # max_attn_weights = torch.max(attn_weights, 1)[0]
        # attn_weights = torch.sub(attn_weights, max_attn_weights.unsqueeze(1))
        attn_weights=attn_weights_o[:, :, 1:]

        ### attention visulization
        # attn_weights1 = attn_weights_o[:, 1:, :]
        # scaled_attn_weights1 = torch.divide(attn_weights1, math.sqrt(l))
        # scaled_attn_weights1 = torch.tril(scaled_attn_weights1)
        # scaled_attn_weights1[scaled_attn_weights1 == 0] = -1000
        # soft_attn_weights1 = F.softmax(scaled_attn_weights1, dim=-1)
        # soft_attn_weights1= torch.tril(soft_attn_weights1)
        ### attention visulization end
        ...
        # soft_attn_weights = torch.tril(soft_attn_weights)

        soft_attn_weights = F.softmax(scaled_attn_weights, dim=-2)
        soft_attn_weights= torch.triu(soft_attn_weights)
        # print(state.transpose(1, 2).size() )
