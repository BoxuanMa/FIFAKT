...
def extract_tag_skills_tage(word_df, language_df, lemma=False):
    skill_df = []
...
def prepare_tagetomo(min_interactions_per_user, remove_nan_skills, subword_skills=False, tags=False, lemma=False, drop_duplicates=False):
    ...
    if subword_skills:
        ...
        df["skill_id"] = extract_tag_skills_tage(df["tag"], df["learning_language"], lemma=lemma)
    elif lemma:
        df["skill_id"] = df["tag"].apply(lambda x: x.split("/")[1].split("<")[0])
    else:
        df["skill_id"] = df["word"]

    df["item_id"] = df["format"].apply(str) + ":" + df["word"]
